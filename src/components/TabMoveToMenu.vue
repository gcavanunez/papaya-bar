...
<template>
	<div class="relative">
		<Menu v-slot="{ open }">
						...
						<AppButton ref="trigger" type="button" size="x-small" intent="common">
							<slot name="menu-trigger-label">
								<span class="inline-flex items-center">
									Move to
									<ChevronDownIcon class="-mr-1 ml-1 h-2.5 w-2.5 text-slate-800 dark:text-white"
										aria-hidden="true" />
								</span>
							</slot>
						</AppButton>
			...
			> -->
			<div ref="container" class="z-30 w-56" :style="{
				position: strategy,
				transform: `translate3d(${Math.round(x ?? 0)}px,${Math.round(y ?? 0)}px,0)`,
				top: '0',
				left: '0',
				...
				<transition enter-active-class="transition duration-100 ease-out"
					...
					<MenuItems
						...
						<div class="px-1 py-1">
							...
							<MenuItem v-for="[windowId, value] in [...windowsMap]" v-slot="{ active, disabled }"
								:key="`${windowId}-enabled-windows`" @click="
									moveTabTo(tabs, {
						...
						<div v-if="loadedGroups.length" class="px-1 py-1">
							<MenuItem v-for="loadedGroup in loadedGroups" v-slot="{ active, disabled }"
								:key="`${loadedGroup.id}-custom-selected`" @click="
									moveTabTo(tabs, {
