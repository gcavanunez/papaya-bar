	...
	const openAll = async ({
		...
		const windoPromises = [...groupedTabs.entries()].map(([id, value]) => {
			...
			if (firstTabInWindow.groupId) {
				shouldBeLaterGroupedTabs.set(firstTabInWindow.groupId, [
					firstTabInWindow,
				]);
			}
			...
			const scale = Math.min(
				loadDisplays[0].width / width!,
				loadDisplays[0].height / height!,
			);
				...
				return row.tabs
					...
					.map((row) => {
						...
						if (row.groupId) {
							...
							shouldBeLaterGroupedTabs.set(row.groupId, [
								...alreadyGroupedTabs,
								row,
							]);
						}
		...
		groupedByGroups.forEach((lilGroup) => {
			const groupableTabs = shouldBeLaterGroupedTabs.get(
				lilGroup.group.id,
			);
