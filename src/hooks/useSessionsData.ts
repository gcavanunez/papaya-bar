...
export const useSessionsData = () => {
	...
	const openAll = async ({
		...
		let loadDisplays: SavedDisplay[] = await new Promise((resolve) => {
			chrome.system.display.getInfo((display) => {
				resolve(
					display.map(({ bounds: { height, left, top, width }, id, name, rotation }) => {
						console.log('exta info that might help:', { id, name, rotation })
		...
		const groupedByGroups = tabs.reduce((acc, curr) => {
			if (curr.groupId) {
				...
				if (data) {
					const alreadyGrouped = acc.get(curr.groupId) || { group: data, tabs: [] }
		...
		const windoPromises = [...groupedTabs.entries()].map(([id, value]) => {
			...
			const fits = loadDisplays.some((row) => {
				console.log(
					...
					areRectanglesOverlap([row.left, row.height, row.width, row.top], windowCoordinates)
				)
				return areRectanglesOverlap([row.left, row.height, row.width, row.top], windowCoordinates)
			})
			...
			if (fits) {
				return chrome.windows.create({ height, left, top, width, url: firstTabInWindow.url })
			}
		...
		const coolWindowData = windows.map((newChromeWindow, index) => {
			...
			return {
				...
				tabs: lilTabGroup.map((row) => ({ ...row, newWindowId: newChromeWindow.id })),
			...
			coolWindowData.flatMap((row) => {
				...
				return row.tabs
					...
					.map((row) => {
						...
						if (row.groupId) {
							const alreadyGroupedTabs = shouldBeLaterGroupedTabs.get(row.groupId) || []
